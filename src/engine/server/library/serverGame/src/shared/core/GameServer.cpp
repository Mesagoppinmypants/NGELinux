...
void GameServer::receiveMessage(const MessageDispatch::Emitter & source, const MessageDispatch::MessageBase & message)
{
	...
	else if (message.isType("CustomerServiceServerGameServerServiceAddress"))
	{
		...
		m_customerServiceServerConnection = new CustomerServiceServerConnection(address.getValue().first, address.getValue().second);
	}
	else if(message.isType("BaselinesMessage"))
	{
		...
		else
		{
			...
			if (so)
			{
				DEBUG_WARNING(true, ("Applied baselines on object %s which is already initialized.", m.getTarget().getValueString().c_str()));
				so->applyBaselines(m);
			}
			else
			{
				DEBUG_WARNING(true, ("Got baselines on object %s which could not be found.", m.getTarget().getValueString().c_str()));
			}
		}
	}
	else if(message.isType("BatchBaselinesMessage"))
	{
		MESSAGE_PROFILER_BLOCK("BaselinesMessage");
		ri = static_cast<GameNetworkMessage const &>(message).getByteStream().begin();
		...
		for (std::vector<BatchBaselinesMessageData>::const_iterator i=baselines.begin(); i!=baselines.end(); ++i)
		{
			...
			if (!lastObject || lastObject->getNetworkId() != i->m_networkId)
			{
				...
				if (!lastObject)
				{
					...
					if (lastObject)
					{
						WARNING(true, ("Applied baselines on object %s which is already initialized.", i->m_networkId.getValueString().c_str()));

					}
					else
					{
						WARNING(true, ("Got baselines on object %s which could not be found.", i->m_networkId.getValueString().c_str()));
					}
				}
			}
			if (lastObject)
	...
	else if(message.isType("DeltasMessage"))
	{
		...
		if (so)
			...
			DEBUG_WARNING(true, ("Got DeltasMessage on object %s, which could not be found.", m.getTarget().getValueString().c_str()));
	}
	...
	else if(message.isType("ChunkCompleteMessage"))
	{
		...
		for (std::vector<std::pair<int, int> >::const_iterator i=chunks.begin(); i!=chunks.end(); ++i)
	...
	else if (message.isType("AddGameServer"))
	{
		...
		FATAL(std::find(m_gameServerPids.begin(), m_gameServerPids.end(), pid) != m_gameServerPids.end(), ("Got AddGameServer for server %lu which we thought was already running?", pid));
	...
	else if (message.isType("ExcommunicateGameServerMessage"))
	{
		...
		FATAL(pid == getProcessId(), ("Crashing because Central told us to (probably indicates we weren't responding to pings)"));
	...
	else if (message.isType("ConnectionServerAddress"))
	{
		...
		ConnectionServerConnection * const newConn = new ConnectionServerConnection(msg.getGameServiceAddress(), msg.getGameServicePort());
		m_connectionServerVector->push_back(newConn);
	}
	else if (message.isType("ConnectionServerConnectionClosed") || message.isType("ConnectionServerConnectionDestroyed"))
	{
		...
		ConnectionServerVector::iterator i = std::find(m_connectionServerVector->begin(), m_connectionServerVector->end(), c);
		if (i != m_connectionServerVector->end())
		{
			m_connectionServerVector->erase(i);
		}
	}
	else if(message.isType("NewClient"))
	{
		...
		if (m_clients->find(msg.getNetworkId()) != m_clients->end())
		{
			...
			LOG(loginTrace, ("NewClient(%s) : Error!  Getting a NewClient message for a client that already exists", msg.getNetworkId().getValueString().c_str()));
		...
		if (obj)
		{
			...
			if (objVars.getItem("disableLoginCtsInProgress", timeOut))
			{
				...
				{
					MessageToQueue::getInstance().sendMessageToC(obj->getNetworkId(), "C++kickPlayerCtsCompletedOrInProgress", "", 0, false);
					kickRequested = true;
				}
				else
				{
			...
			int transferredTime;
			if (!kickRequested && objVars.getItem("disableLoginCtsCompleted", transferredTime) && !msg.getUsingAdminLogin())
			{
				MessageToQueue::getInstance().sendMessageToC(obj->getNetworkId(), "C++kickPlayerCtsCompletedOrInProgress", "", 0, false);
				kickRequested = true;
			}
		...
		LOG(loginTrace, ("NewClient(%s) : A new client is being created from connection server", msg.getNetworkId().getValueString().c_str()));
		std::set<NetworkId> const observedObjectSet(msg.getObservedObjects().begin(), msg.getObservedObjects().end());
		...
		{
			AccountFeatureIdRequest const req(NetworkId::cms_invalid, GameServer::getInstance().getProcessId(), msg.getNetworkId(), static_cast<StationId>(msg.getStationId()), PlatformGameCode::SWG, AccountFeatureIdRequest::RR_Reload);
			c->sendToConnectionServer(req);
		}
	}
	else if(message.isType("AuthTransferClientMessage"))
	{
		...
		{
			// inconsistant state, drop the client
			LOG(loginTrace, ("AuthTransferClientMessage(%s) : Error!  Getting a AuthTransferClientMessage message for a client that already exists", msg.getNetworkId().getValueString().c_str()));
			KickPlayer const kickMessage(msg.getNetworkId(), "Duplicate Client");
			sendToConnectionServers(kickMessage);
			dropClient(msg.getNetworkId());
			return;
		...
		ConnectionServerConnection * const connectionServer = getConnectionServerConnection(msg.getConnectionServerIp(), msg.getConnectionServerPort());

		if (!connectionServer)
		{
			// inconsistant state, drop the client
			LOG(loginTrace, ("AuthTransferClientMessage(%s) : Error!  Getting a AuthTransferClientMessage message for a client but could not locate connection server (%s:%hu)", msg.getNetworkId().getValueString().c_str(), msg.getConnectionServerIp().c_str(), msg.getConnectionServerPort()));
		...
		LOG(loginTrace, ("AuthTransferClientMessage(%s) : A new client is being created from game server %lu", msg.getNetworkId().getValueString().c_str(), msg.getSourceServerPid()));

		std::set<NetworkId> observedObjectSet(msg.getObservedObjects().begin(), msg.getObservedObjects().end());
		Client * c = new Client(*connectionServer, msg.getNetworkId(), msg.getAccount(), msg.getIpAddress(), msg.getSecure(), msg.getSkipLoadScreen(), msg.getStationId(), observedObjectSet, msg.getGameFeatures(), msg.getSubscriptionFeatures(), msg.getAccountFeatureIds(), msg.getEntitlementTotalTime(), msg.getEntitlementEntitledTime(), msg.getEntitlementTotalTimeSinceLastLogin(), msg.getEntitlementEntitledTimeSinceLastLogin(), msg.getBuddyPoints(), msg.getConsumedRewardEvents(), msg.getClaimedRewardItems(), msg.getUsingAdminLogin(), static_cast<CombatDataTable::CombatSpamFilterType>(msg.getCombatSpamFilter()), msg.getCombatSpamRangeSquaredFilter(), msg.getFurnitureRotationDegree(), msg.getHasUnoccupiedJediSlot(), msg.getIsJediSlotCharacter());
		ClientMap::iterator f = m_clients->find(msg.getNetworkId());
	...
	else if(message.isType("CentralGameServerSetProcessId"))
	{
		...
		DEBUG_FATAL(m.getClockSubtractInterval()==0,("Got 0 for the clock subtract interval (probably indicates we connected to Central too early).\n"));
	...
	else if (message.isType("SetAuthoritativeMessage"))
	{
		...
		if (object)
		{
			...
			{
			if (t.getGoalIsValid())
				object->transferAuthority(t.getProcess(), !t.getSceneChange(), t.getHandlingCrash(), t.getGoalCell(), t.getGoalTransform(), false);
			else
	...
	else if(message.isType("LoadObjectMessage"))
	{
		...
		if (target)
			...
			WARNING(true, ("Told to create object %s on %d we know nothing about (received LoadObjectMessage for an object not on this server, PlanetServer is probably confused)", t.getId().getValueString().c_str(), t.getProcess()));
	}
		...
		if(object)
		{
			if(object->isAuthoritative())
			{
				if (unloadProxyMessage.getProxyGameServerId() != getProcessId())
					...
					DEBUG_WARNING(true,("Can't unproxy object %s from this server because this server is authoritative.", unloadProxyMessage.getObjectId().getValueString().c_str()));
			}
			else
			{
				DEBUG_REPORT_LOG(ConfigServerGame::getLogObjectLoading(),("Forwarding UnloadProxyMessage for %s to authoritative server\n", unloadProxyMessage.getObjectId().getValueString().c_str()));
			...
			DEBUG_WARNING(true, ("Received UnloadProxyMessage for object %s server %lu, but the object could not be found", unloadProxyMessage.getObjectId().getValueString().c_str(), unloadProxyMessage.getProxyGameServerId()));
	}
	...
	else if(message.isType("ObjControllerMessage"))
	{
		...
		if (target != NULL)
		{
			...
			if (controller != NULL)
			{
				...
				if(flags & GameControllerMessageFlags::DEST_AUTH_SERVER)
				{
					...
					{
						WARNING(! target->isAuthoritative(), ("Received a controller message (%d) destined for the authoritative object (%s:%s) on this server, but the object is NOT authoritative on this server", c.getMessage(), target->getObjectTemplateName(), target->getNetworkId().getValueString().c_str() ));
					}
				}
		...
		else
		{
			...
			if (destServer)
			{
				if(ConfigServerGame::getEnableDebugControllerMessageSpam())
				{
					WARNING(true, ("ObjControllerMessage %d for object %s arrived, but the object has transferred authority to server %d", c.getMessage(), c.getNetworkId().getValueString().c_str(), destServer));
				}
				...
				DEBUG_WARNING(true, ("Got ObjControllerMessage for object %s, which could not be found.", c.getNetworkId().getValueString().c_str()));
		}
	...
	else if(message.isType("EndBaselinesMessage"))
	{
		...
		else
		{
			...
			WARNING(!badObject, ("Got EndBaeslinesMessage on object (%s) that doesn't exist!", t.getId().getValueString().c_str()));
			WARNING(badObject, ("Got EndBaselinesMessage on object (%s : %s) which has already been created and placed in the world!", badObject->getTemplateName(), badObject->getNetworkId().getValueString().c_str()));
		}
	}
	else if(message.isType("TaskShutdownGameServer"))
	{
	...
	else if (message.isType("CreateObjectByCrcMessage"))
	{
		...
		if (object)
		{
			...
			{
				WARNING_STRICT_FATAL(true, ("Hey, Got create message for object %s That isn't being deleted!", networkId.getValueString().c_str()));
				okToCreate = false;
			}
		}
	...
	else if(message.isType("CreateSyncUiMessage"))
	{
	...
	else if(message.isType("SynchronizeScriptVarsMessage"))
	{
	...
	else if (message.isType("SynchronizeScriptVarDeltasMessage"))
	{
		...
		if (so)
		{
			...
			DEBUG_WARNING(true,("Got SynchronizeScriptVarDeltasMessage on object %s, which could not be found.", msg.getNetworkId().getValueString().c_str()));
	}
	...
	else if(message.isType("DropClient"))
	{
	...
	else if(message.isType("SetPlanetServerMessage"))
	{
	...
	else if (message.isType("PlanetConnectionOpened"))
	{
	...
	else if (message.isType("PlanetConnectionClosed"))
	{
	...
	else if (message.isType("GameServerSetupMessage"))
	{
		...
		ri = static_cast<GameNetworkMessage const &>(message).getByteStream().begin();
		GenericValueTypeMessage<std::pair<std::vector<uint32>, std::pair<uint32, std::pair<std::string, uint16> > > > const m(ri);
	...
	else if (message.isType("SetUniverseAuthoritativeMessage"))
	{
	...
	else if (message.isType("AddImportedResourceType"))
	{
		ri = static_cast<GameNetworkMessage const &>(message).getByteStream().begin();
	...
	else if (message.isType("UniverseCompleteMessage"))
	{
	...
	else if (message.isType("PreloadRequestCompleteMessage"))
	{
		...
		LOG("Preload", ("Game Server %d received preloadRequestCompleteMessage from database", getInstance().m_processId));
		ri = static_cast<GameNetworkMessage const &>(message).getByteStream().begin();
		PreloadRequestCompleteMessage const msg(ri);

		DEBUG_REPORT_LOG(true,("Got PreloadRequestCompleteMessage.  We are server %lu in the preload list\n",msg.getPreloadAreaId()));

		m_preloadAreaId = msg.getPreloadAreaId();
		MetricsManager::install(m_metricsData, true, "GameServer", ServerWorld::getSceneId(), msg.getPreloadAreaId());
		...
		if (static_cast<int>(msg.getPreloadAreaId()) != ConfigServerGame::getPreloadNumber())
			LOG("Preload", ("Game Server %d got assigned preload number %lu but was launched with preload number %d", getInstance().m_processId, msg.getPreloadAreaId(), ConfigServerGame::getPreloadNumber()));
#endif
	...
	else if (message.isType("SceneTransferMessage"))
	{
		...
		if (obj && obj->isAuthoritative() && isGameServerConnected(msg.getDestinationGameServer()) && (!obj->isPlayerControlled() || obj->getClient()))
		{
			...
			else
			{
				...
				if (doSceneChange)
				{
					...
					GenericValueTypeMessage<std::pair<NetworkId, std::pair<std::string, bool> > > const setSceneMsg("SetSceneForPlayer", std::make_pair(msg.getNetworkId(), std::make_pair(msg.getSceneName(), false)));
					...
					if (!msg.getSceneName().empty())
					{
						...
						std::map<NetworkId, LfgCharacterData>::const_iterator iterFind = connectedCharacterLfgData.find(msg.getNetworkId());
						if ((iterFind != connectedCharacterLfgData.end()) && (iterFind->second.locationPlanet != msg.getSceneName()))
					...
					if (msg.getContainerId() != NetworkId::cms_invalid)
						obj->setInteriorTeleportDestination(msg.getContainerId(), msg.getContainerName(), msg.getPosition_p());
	...
	else if(message.isType("TeleportMessage"))
	{
	...
	else if (message.isType("TeleportToMessage"))
	{
		MESSAGE_PROFILER_BLOCK("TeleportToMessage");
		ri = static_cast<GameNetworkMessage const &>(message).getByteStream().begin();
		TeleportToMessage const msg(ri);
		ServerObject * const serverObj = safe_cast<ServerObject *>(NetworkIdManager::getObjectById(msg.getTargetId()));
	...
	else if(message.isType("RetrievedItemLoadMessage"))
	{
	...
	else if(message.isType("LoadContainedObjectMessage"))
	{
		...
		if (obj)
		{
			WARNING(!obj->isAuthoritative(), ("Got LoadContainedObjectMessage, but the container is not authoritative.  Container is %s, Object id %s",
	...
	else if(message.isType("LoadContentsMessage"))
	{
		...
		if (obj)
		{
			WARNING(!obj->isAuthoritative(),("Got LoadContentsMessage, but the container is not authoritative.  Container is %s",
				obj->getDebugInformation().c_str()));
	...
	else if(message.isType("FirstPlanetGameServerIdMessage"))
	{
		...
		DEBUG_REPORT_LOG(true,("First game server for %s is %lu\n",ServerWorld::getSceneId().c_str(),m_firstGameServerForPlanet));
	}
	else if(message.isType("CreateDynamicRegionCircleMessage"))
	{
		...
		RegionMaster::createNewDynamicRegion(msg.getCenterX(), msg.getCenterZ(), msg.getRadius(), msg.getName(), msg.getPlanet(), msg.getPvp(), msg.getBuildable(), msg.getMunicipal(), msg.getGeography(), msg.getMinDifficulty(), msg.getMaxDifficulty(), msg.getSpawnable(), msg.getMission(), msg.getVisible(), msg.getNotify());
	}
	else if(message.isType("CreateDynamicRegionRectangleMessage"))
	{
		...
		RegionMaster::createNewDynamicRegion(msg.getMinX(), msg.getMinZ(), msg.getMaxX(), msg.getMaxZ(), msg.getName(), msg.getPlanet(), msg.getPvp(), msg.getBuildable(), msg.getMunicipal(), msg.getGeography(), msg.getMinDifficulty(), msg.getMaxDifficulty(), msg.getSpawnable(), msg.getMission(), msg.getVisible(), msg.getNotify());
	}
	...
	else if(message.isType("ReloadAdminTableMessage"))
	{
	...
	else if(message.isType("ReloadCommandTableMessage"))
	{
	...
	else if (message.isType(ReloadDatatableMessage::ms_messageName))
	{
	...
	else if(message.isType("ReloadScriptMessage"))
	{
	...
	else if(message.isType("ReloadTemplateMessage"))
	{
	...
	else if(message.isType("EnableNewJediTrackingMessage"))
	{
...
void GameServer::receiveMessage2(const MessageDispatch::Emitter & source, const MessageDispatch::MessageBase & message)
{
	...
	if(message.isType("AuthTransferConfirmMessage"))
	{
	...
	else if (message.isType("LocateObject"))
	{
		ri = static_cast<GameNetworkMessage const &>(message).getByteStream().begin();
		...
		ServerObject const * const targetObj = safe_cast<ServerObject const *>(NetworkIdManager::getObjectById(targetId));
		if (targetObj)
		{
	...
	else if (message.isType("LocateObjectByTemplateName"))
	{
		ri = static_cast<GameNetworkMessage const &>(message).getByteStream().begin();
		...
		for (NetworkIdManager::NetworkIdObjectHashMap::const_iterator iter = allObjects.begin(); iter != allObjects.end(); ++iter)
		{
			...
			{
				std::vector<NetworkId> containers;
				for (Object const *o = ContainerInterface::getContainedByObject(*(iter->second)); o; o = ContainerInterface::getContainedByObject(*o))
					containers.push_back(o->getNetworkId());
	...
	else if (message.isType("LocatePlayerByPartialName"))
	{
		ri = static_cast<GameNetworkMessage const &>(message).getByteStream().begin();
		...
		if (!players.empty())
		{
			...
			for (i = players.begin(); i != players.end(); ++i)
			{
				...
				if (!creatureObject || !creatureObject->isInWorld() || !creatureObject->isAuthoritative() || !creatureObject->getClient())
					continue;

				// Lower case the first name of the player for the comparison
				Unicode::String lowerCasePlayerName = Unicode::toLower(creatureObject->getAssignedObjectFirstName());
	...
	else if (message.isType("LocateWarden"))
	{
		ri = static_cast<GameNetworkMessage const &>(message).getByteStream().begin();
	...
	else if (message.isType("LocateCreatureByCreatureName"))
	{
		ri = static_cast<GameNetworkMessage const &>(message).getByteStream().begin();
		GenericValueTypeMessage<std::pair<uint32, std::pair<std::string, NetworkId> > > const msg(ri);
	...
	else if (message.isType("LSBOIReq"))
	{
		ri = static_cast<GameNetworkMessage const &>(message).getByteStream().begin();
		...
		handleMessageLocateStructureByOwnerIdReq(*this, locateStructureByOwnerIdReq.getValue().first.first, locateStructureByOwnerIdReq.getValue().first.second, locateStructureByOwnerIdReq.getValue().second.first.first, locateStructureByOwnerIdReq.getValue().second.first.second, locateStructureByOwnerIdReq.getValue().second.second);
	}
	else if (message.isType("LSBOIRsp"))
	{
		ri = static_cast<GameNetworkMessage const &>(message).getByteStream().begin();
		GenericValueTypeMessage<std::pair<std::pair<uint32, NetworkId>, std::vector<std::string> > > const locateStructureByOwnerIdRsp(ri);

		handleMessageLocateStructureByOwnerIdRsp(locateStructureByOwnerIdRsp.getValue().first.second, locateStructureByOwnerIdRsp.getValue().second);
	}
	else if (message.isType("ReportSystemClockTime"))
	{
		ri = static_cast<GameNetworkMessage const &>(message).getByteStream().begin();
		GenericValueTypeMessage<std::pair<uint32, NetworkId> > const msg(ri);
	...
	else if (message.isType("ReportSystemClockTimeResponse"))
	{
		ri = static_cast<GameNetworkMessage const &>(message).getByteStream().begin();
		GenericValueTypeMessage<std::pair<uint32, std::pair<std::string, NetworkId> > > const msg(ri);
	...
	else if (message.isType("ReportPlanetaryTime"))
	{
		ri = static_cast<GameNetworkMessage const &>(message).getByteStream().begin();
		GenericValueTypeMessage<std::pair<uint32, NetworkId> > const msg(ri);
		...
		if (terrainObject)
		{
			...
			{
				const float day = fmodf(static_cast<float>(ServerClock::getInstance().getGameTimeSeconds()), environmentCycleTime) / environmentCycleTime;
				int hour   = 6 + static_cast<int>(day * 24.f * 60.f) / 60;
				if (hour >= 24)
					hour -= 24;
				const int minute = static_cast<int>(fmodf(day * 24.f * 60.f, 60.f));

				time += FormattedString<1024>().sprintf(" (%g, %02d:%02d, %f)", environmentCycleTime, hour, minute, day);
			}
			else
			{
				time += FormattedString<1024>().sprintf(" (environmentCycleTime %g is <= 0.f)", environmentCycleTime);
			}
		}
		else
		{
		...
		GenericValueTypeMessage<std::pair<uint32, std::pair<std::string, NetworkId> > > rptr(
			"ReportPlanetaryTimeResponse", std::make_pair(msg.getValue().first, std::make_pair(time, msg.getValue().second)));

		sendToCentralServer(rptr);
	}
	else if (message.isType("ReportPlanetaryTimeResponse"))
	{
		ri = static_cast<GameNetworkMessage const &>(message).getByteStream().begin();
		GenericValueTypeMessage<std::pair<uint32, std::pair<std::string, NetworkId> > > const msg(ri);
	...
	else if (message.isType("ClusterStartComplete"))
	{
		ri = static_cast<GameNetworkMessage const &>(message).getByteStream().begin();
		...
		if (msg.getValue())
		{
			...
			if (ServerUniverse::getInstance().isAuthoritative())
			{
				...
				if (go && go->isAuthoritative() && co && co->isAuthoritative())
				{
					...
					std::map<std::string, int> const & gcwImperialScorePercentile = go->getGcwImperialScorePercentile();
					...
					else
					{
						...
						for (std::map<std::string, int>::const_iterator iter = gcwImperialScorePercentile.begin(); iter != gcwImperialScorePercentile.end(); ++iter)
							go->updateGcwRegionDefenderBonus(iter->first);
					}
				}
			}

			if (ConfigServerGame::getEnableCityCitizenshipFixup() && ServerUniverse::getInstance().isAuthoritative())
			{
				...
				if (!allGameServerPids.empty())
				{
					...
					for (std::vector<uint32>::const_iterator iter = allGameServerPids.begin(); iter != allGameServerPids.end(); ++iter)
	...
	else if (message.isType("CSRSLReq"))
	{
		ri = static_cast<GameNetworkMessage const &>(message).getByteStream().begin();
		...
		if (!s_clusterStartupResidenceStructureListByStructure.empty())
		{
			for (std::map<NetworkId, std::pair<int, NetworkId> >::const_iterator iter = s_clusterStartupResidenceStructureListByStructure.begin(); iter != s_clusterStartupResidenceStructureListByStructure.end(); ++iter)
				IGNORE_RETURN(clusterStartupResidenceStructureListByCity[iter->second.first].insert(iter->second.second));
		}

		GenericValueTypeMessage<std::pair<uint32, std::map<int, std::set<NetworkId> > > > clusterStartupResidenceStructureListResponse("CSRSLRsp", std::make_pair(getProcessId(), clusterStartupResidenceStructureListByCity));
	...
	else if (message.isType("CSRSLRsp"))
	{
		ri = static_cast<GameNetworkMessage const &>(message).getByteStream().begin();
		GenericValueTypeMessage<std::pair<uint32, std::map<int, std::set<NetworkId> > > > const clusterStartupResidenceStructureListResponse(ri);

		if (s_clusterStartupResidenceStructureListResponse.erase(clusterStartupResidenceStructureListResponse.getValue().first) != 0)
		{
			static std::map<int, std::set<NetworkId> > s_clusterStartupResidenceStructureListByCity;

			std::map<int, std::set<NetworkId> > const & residenceList = clusterStartupResidenceStructureListResponse.getValue().second;
			if (!residenceList.empty())
			{
				for (std::map<int, std::set<NetworkId> >::const_iterator iter = residenceList.begin(); iter != residenceList.end(); ++iter)
				{
					if (!iter->second.empty())
						s_clusterStartupResidenceStructureListByCity[iter->first].insert(iter->second.begin(), iter->second.end());
				}
			}
			...
			if (s_clusterStartupResidenceStructureListResponse.empty())
			{
				...
				{
					for (std::map<NetworkId, std::pair<int, NetworkId> >::const_iterator iter = s_clusterStartupResidenceStructureListByStructure.begin(); iter != s_clusterStartupResidenceStructureListByStructure.end(); ++iter)
						IGNORE_RETURN(s_clusterStartupResidenceStructureListByCity[iter->second.first].insert(iter->second.second));
				}
				...
				{				
					...
					for (std::map<int, CityInfo>::const_iterator iterAllCities = allCities.begin(); iterAllCities != allCities.end(); ++iterAllCities)
					{
						if (s_clusterStartupResidenceStructureListByCity.count(iterAllCities->first) <= 0)
							IGNORE_RETURN(s_clusterStartupResidenceStructureListByCity.insert(std::make_pair(iterAllCities->first, std::set<NetworkId>())));

						if (iterAllCities->second.getLeaderId().isValid())
							allCityMayors[iterAllCities->second.getLeaderId()] = std::make_pair(iterAllCities->first, iterAllCities->second.getCityName());
					}
					...
					for (std::map<int, std::set<NetworkId> >::iterator iterCityId = s_clusterStartupResidenceStructureListByCity.begin(); iterCityId != s_clusterStartupResidenceStructureListByCity.end(); ++iterCityId)
					{
						...
						LOG("CityFixup", ("City %d (%s:%s, %s:%s) has %d declared structures within its city limits.", iterCityId->first, cityName.c_str(), cityInfo.getCityHallId().getValueString().c_str(), currentCityMayorName.c_str(), currentCityMayor.getValueString().c_str(), iterCityId->second.size()));
						...
						static const Unicode::String mailSubjectRemove = Unicode::narrowToWide("@" + StringId("city/city", "city_fixup_remove_citizens_subject").getCanonicalRepresentation());
						citizensToRemove.clear();
						citizensToRemoveNames.clear();
						citizensToRemoveDeletedNames.clear();
						citizensToRemoveInactiveNames.clear();
						for (std::map<std::pair<int, NetworkId>, CitizenInfo>::const_iterator iterCurrentCitizen = allCitizens.lower_bound(std::make_pair(iterCityId->first, NetworkId::cms_invalid)); iterCurrentCitizen != allCitizens.end(); ++iterCurrentCitizen)
						{
							...
							hasDeclaredResidence = (iterCityId->second.erase(iterCurrentCitizen->first.second) >= 1);
							...
							if (iterCurrentCitizen->first.second != currentCityMayor)
							{
								if (!hasDeclaredResidence)
								{
									...
									LOG("CustomerService", ("CityFixup: removed %s (%s) (no declared residence) as citizen of city %d (%s).", iterCurrentCitizen->first.second.getValueString().c_str(), citizenName.c_str(), iterCityId->first, cityName.c_str()));
								}
								else if (allCityMayors.count(iterCurrentCitizen->first.second) >= 1)
								{
									...
									removeCurrentCitizen = true;
									LOG("CustomerService", ("CityFixup: removed %s (%s) (already mayor of %d:%s) as citizen of city %d (%s).", iterCurrentCitizen->first.second.getValueString().c_str(), citizenName.c_str(), allCityMayors[iterCurrentCitizen->first.second].first, allCityMayors[iterCurrentCitizen->first.second].second.c_str(), iterCityId->first, cityName.c_str()));
								}
								else
								{
									citizenLastLoginTime = NameManager::getInstance().getPlayerLastLoginTime(iterCurrentCitizen->first.second);
									if (citizenLastLoginTime <= 0)
									{
										// remove citizen because he no longer exists in the DB
										removeCurrentCitizen = true;
										removeCurrentCitizenDeleted = true;
										LOG("CustomerService", ("CityFixup: removed %s (%s) (deleted) as citizen of city %d (%s).", iterCurrentCitizen->first.second.getValueString().c_str(), citizenName.c_str(), iterCityId->first, cityName.c_str()));
									}
									else if (citizenInactivePackupActive && ((citizenLastLoginTime + ConfigServerGame::getCityCitizenshipInactivePackupInactiveTimeSeconds()) <= timeNow) && !(iterCurrentCitizen->second.m_citizenPermissions & CitizenPermissions::InactiveProtected))
									{
										...
										LOG("CustomerService", ("CityFixup: removed %s (%s) (inactive %s) as citizen of city %d (%s).", iterCurrentCitizen->first.second.getValueString().c_str(), citizenName.c_str(), CalendarTime::convertSecondsToDHMS(static_cast<unsigned int>(timeNow - citizenLastLoginTime)).c_str(), iterCityId->first, cityName.c_str()));
									}
								}
							}
							...
							if (removeCurrentCitizen)
							{
								...
								if (!currentCityMayorName.empty())
								{
									...
									else if (removeCurrentCitizenInactive)
									{
										...
										citizensToRemoveInactiveNames += CalendarTime::convertSecondsToDHMS(static_cast<unsigned int>(timeNow - citizenLastLoginTime));
										citizensToRemoveInactiveNames += ")";
						...
						// remove citizens
						for (std::vector<NetworkId>::const_iterator iterCitizenToRemove = citizensToRemove.begin(); iterCitizenToRemove != citizensToRemove.end(); ++iterCitizenToRemove)
						{
						...
						// add citizens
						static const Unicode::String mailSubjectAdd = Unicode::narrowToWide("@" + StringId("city/city", "city_fixup_add_citizens_subject").getCanonicalRepresentation());
						citizensToAddNames.clear();
						for (std::set<NetworkId>::const_iterator iterCitizenToAdd = iterCityId->second.begin(); iterCitizenToAdd != iterCityId->second.end(); ++iterCitizenToAdd)
						{
							...
							{
								citizenLastLoginTime = NameManager::getInstance().getPlayerLastLoginTime(*iterCitizenToAdd);
								if ((citizenLastLoginTime <= 0) || ((citizenLastLoginTime + ConfigServerGame::getCityCitizenshipInactivePackupInactiveTimeSeconds()) <= timeNow))
									continue;
							}

							// don't add someone who is already a mayor of another city, unless
							// we are adding this city's mayor as a citizen of this city
							if ((*iterCitizenToAdd != currentCityMayor) && (allCityMayors.count(*iterCitizenToAdd) >= 1))
								continue;
							...
							if (*iterCitizenToAdd == currentCityMayor)
								LOG("CustomerService", ("CityFixup: added %s (%s) (is mayor) as citizen of city %d (%s).", iterCitizenToAdd->getValueString().c_str(), citizenName.c_str(), iterCityId->first, cityName.c_str()));
							else
								LOG("CustomerService", ("CityFixup: added %s (%s) (has declared residence) as citizen of city %d (%s).", iterCitizenToAdd->getValueString().c_str(), citizenName.c_str(), iterCityId->first, cityName.c_str()));

							CityInterface::setCitizenInfo(iterCityId->first, *iterCitizenToAdd, citizenName, NetworkId::cms_invalid, CitizenPermissions::Citizen);
						}
	...
	else if (message.isType("LoadStructureMessage"))
	{
		ri = static_cast<GameNetworkMessage const &>(message).getByteStream().begin();
		...
		else
		{
			...
			std::string const replyMessage = "Structure " + msg.getStructureId().getValueString() + " is being loaded";
			Chat::sendSystemMessage(msg.getWhoRequested(), Unicode::narrowToWide(replyMessage), Unicode::String());
		}
	}
	else if (message.isType("LocateObjectResponseMessage"))
	{
		ri = static_cast<GameNetworkMessage const &>(message).getByteStream().begin();
		LocateObjectResponseMessage const msg(ri);

		ServerObject * const responseObject = safe_cast<ServerObject *>(NetworkIdManager::getObjectById(msg.getResponseId()));
		if (responseObject && responseObject->isAuthoritative())
		{
			int const cityId = CityInterface::getCityAtLocation(msg.getScene(), static_cast<int>(msg.getPosition_w().x), static_cast<int>(msg.getPosition_w().z), 0);
	...
	else if (message.isType("LocatePlayerResponseMessage"))
	{
		ri = static_cast<GameNetworkMessage const &>(message).getByteStream().begin();
		...
		{
			std::string playerInfo = FormattedString<512>().sprintf("%s (%s) is on server %lu. %s (%.2f, %.2f, %.2f)",
	...
	else if(message.isType("DatabaseConsoleReplyMessage"))
	{
		...
		Chat::sendSystemMessage(msg.getValue().first,Unicode::narrowToWide(msg.getValue().second),Unicode::String());
	}
	else if(message.isType("FindAuthObject"))
	{
		ri = static_cast<GameNetworkMessage const &>(message).getByteStream().begin();
		GenericValueTypeMessage<std::pair<NetworkId, unsigned int> > const msg(ri);
		ServerObject const * const so = safe_cast<ServerObject const *>(NetworkIdManager::getObjectById(msg.getValue().first));
		bool const found = so && so->isAuthoritative();

		GenericValueTypeMessage<std::pair<std::pair<NetworkId, unsigned int>, bool> > const authResponse(
			"FindAuthObjectResponse",
			std::make_pair(msg.getValue(), found));
		sendToPlanetServer(authResponse);
	}
	else if(message.isType("CharacterNamesMessage"))
	{
		ri = static_cast<GameNetworkMessage const &>(message).getByteStream().begin();
		CharacterNamesMessage const msg(ri);
		NameManager::getInstance().addPlayers(msg.getIds(), msg.getStationIds(), msg.getNames(), msg.getFullNames(), msg.getCreateTimes(), msg.getLoginTimes());
	}
	else if (message.isType ("LocationResponse"))
	{
		...
		if (!object)
		{
			DEBUG_WARNING (true, ("LocationResponse: could not resolve object %s which asked for a location", locationResponse.getNetworkId ().getValueString ().c_str ()));
			return;
		}
		...
		if (!scriptObject)
		{
			DEBUG_WARNING (true, ("LocationResponse: object id=%s template=%s is not a server object", object->getNetworkId ().getValueString ().c_str (), object->getObjectTemplateName ()));
			return;
		}
		...
		if (locationResponse.getValid ())
		{
			...
			snprintf (objectTemplateName, 512, "object/tangible/location/location_%i.iff", static_cast<int> (locationResponse.getRadius ()));
		...
		if (scriptObject->getScriptObject ()->trigAllScripts (Scripting::TRIG_LOCATION_RECEIVED, scriptParameters) != SCRIPT_CONTINUE)
			DEBUG_REPORT_LOG (true, ("LocationResponse: TRIG_LOCATION_RECEIVED: did not return SCRIPT_CONTINUE\n"));
	}
	else if (message.isType("ReleaseCharacterNameByIdMessage"))
	{
		ri = static_cast<GameNetworkMessage const &>(message).getByteStream().begin();
		GenericValueTypeMessage<NetworkId> const msg(ri);
	...
	else if (message.isType("AddAttribModName"))
	{
		ri = static_cast<GameNetworkMessage const &>(message).getByteStream().begin();
	...
	else if (message.isType("AddAttribModNamesList"))
	{
		ri = static_cast<GameNetworkMessage const &>(message).getByteStream().begin();
		GenericValueTypeMessage<std::string> const msg(ri);
	...
	else if(ClusterWideDataClient::handleMessage(source, message))
	{
		// nothing else to do with the message since it was
		// handled by the Cluster wide data client
	}
	else if (message.isType("AiCreatureStateMessage"))
	{
		ri = static_cast<GameNetworkMessage const &>(message).getByteStream().begin();
		...
		if (creatureObject != NULL)
		{
			...
			if (controller != NULL)
			{
				...
				int const flags = GameControllerMessageFlags::SEND | GameControllerMessageFlags::RELIABLE | GameControllerMessageFlags::DEST_SERVER;
			...
			{
				WARNING(true, ("GameServer::receiveMessage() Received AiCreatureStateMessage for networkId(%s) movementType(%s) but the object does not have a Controller", networkId.getValueString().c_str(), AiMovementBase::getMovementString(movement.getMovementType())));
			}
		}
		else
		{
			WARNING(true, ("GameServer::receiveMessage() Received AiCreatureStateMessage for networkId(%s) movementType(%s) but could not resolve to a CreatureObject", networkId.getValueString().c_str(), AiMovementBase::getMovementString(movement.getMovementType())));
		}
	}
	else if (message.isType("UnloadPersistedCharacter"))
	{
		Archive::ReadIterator ri = static_cast<GameNetworkMessage const &>(message).getByteStream().begin();
		...
		if (object)
		{
			WARNING(true, ("Unloaded character object (%s) which has been persisted but still exists on this game server", object->getDebugInformation().c_str()));
			object->unload();
		}
	}
	else if (message.isType("AboutToLoadCharacterFromDB"))
	{
		Archive::ReadIterator ri = static_cast<GameNetworkMessage const &>(message).getByteStream().begin();
		...
		if (object)
		{
			WARNING(true, ("Unloaded character object (%s) because the character object is about to be loaded from the DB onto game server (%lu), I am game server (%lu)", object->getDebugInformation().c_str(), atlcfdb.getValue().second, getProcessId()));
			object->unload();
		}
	}
	else if (message.isType("ClearTheaterMessage"))
	{
		Archive::ReadIterator ri = static_cast<GameNetworkMessage const &>(message).getByteStream().begin();
	...
	else if (message.isType("SetTheaterMessage"))
	{
		Archive::ReadIterator ri = static_cast<GameNetworkMessage const &>(message).getByteStream().begin();
	...
	else if (message.isType("PlayedTimeAccumMessage"))
	{
		Archive::ReadIterator ri = static_cast<GameNetworkMessage const &>(message).getByteStream().begin();
	...
	else if (message.isType("ManualDepleteResourceMessage"))
	{
		ri = static_cast<GameNetworkMessage const &>(message).getByteStream().begin();
		GenericValueTypeMessage<NetworkId> const msg(ri);
	...
	else if (message.isType("ClaimRewardsReplyMessage"))
	{
		...
		VeteranRewardManager::handleClaimRewardsReply(msg.getStationId(), msg.getPlayer(), msg.getRewardEvent(), msg.getRewardItem(), msg.getAccountFeatureId(), msg.getConsumeAccountFeatureId(), msg.getPreviousAccountFeatureIdCount(), msg.getCurrentAccountFeatureIdCount(), msg.getResult(), true);
	}
	else if (message.isType("SetOverrideAccountAgeMessage"))
	{
		ri = static_cast<const GameNetworkMessage &>(message).getByteStream().begin();
	...
	else if (message.isType("GetMoneyFromOfflineObjectMessage"))
	{
		...
		MessageToQueue::getInstance().sendMessageToJava(msg.getReplyTo(), callback, msg.getPackedDictionary(), 0, false);
	}
	...
	else if (message.isType("StructuresForPurgeMessage"))
	{
		ri = static_cast<const GameNetworkMessage &>(message).getByteStream().begin();
		StructuresForPurgeMessage msg(ri);
		PurgeManager::handleStructuresAndVendorsForPurge(msg.getStationId(), msg.getStructures(), msg.getVendors(), msg.getWarnOnly());
	}
	else if (message.isType("RequestLoadAckMessage"))
	{
		ri = static_cast<const GameNetworkMessage &>(message).getByteStream().begin();
		GenericValueTypeMessage<int> msg(ri);
	...
	else if (message.isType("MessageToPlayersOnPlanet"))
	{
		...
		handleMessageToPlayersOnPlanet(msg.getValue().first.first.first, msg.getValue().first.first.second, msg.getValue().first.second.first, msg.getValue().second.first, msg.getValue().second.second, msg.getValue().first.second.second);
	}
	else if (message.isType("PopStatReq"))
	{
		ri = static_cast<GameNetworkMessage const &>(message).getByteStream().begin();
	...
	else if (message.isType("GcwScoreStatReq"))
	{
		ri = static_cast<GameNetworkMessage const &>(message).getByteStream().begin();
		...
		const GenericValueTypeMessage<std::pair<std::map<std::string, int>, std::pair<std::map<std::string, std::pair<int64, int64> >, std::map<std::string, std::pair<int64, int64> > > > > gcwScoreStatisticsResponse("GcwScoreStatRsp", std::make_pair((guildObject ? guildObject->getGcwImperialScorePercentile() : emptyGcwScorePercentile), std::make_pair((tatooine ? tatooine->getGcwImperialScore() : emptyGcwScore), (tatooine ? tatooine->getGcwRebelScore() : emptyGcwScore))));
		sendToCentralServer(gcwScoreStatisticsResponse);
	}
	else if (message.isType("GcwScoreStatRaw"))
	{
		ri = static_cast<GameNetworkMessage const &>(message).getByteStream().begin();
		...
		if (ConfigServerGame::getReceiveGcwScoreFromOtherGalaxies() && ServerUniverse::getInstance().isAuthoritative() && _stricmp(getClusterName().c_str(), msg.getValue().first.c_str()))
		{
			GuildObject * const go = ServerUniverse::getInstance().getMasterGuildObject();
			if (go && go->isAuthoritative())
			{
				go->setGcwRawScoreFromOtherGalaxy(msg.getValue().first, msg.getValue().second.first, msg.getValue().second.second);
			}
		}
	}
	else if (message.isType("GcwScoreStatPct"))
	{
		ri = static_cast<GameNetworkMessage const &>(message).getByteStream().begin();
		...
		if (ConfigServerGame::getReceiveGcwScoreFromOtherGalaxies() && ServerUniverse::getInstance().isAuthoritative() && _stricmp(getClusterName().c_str(), msg.getValue().first.c_str()))
		{
			...
			if (go && go->isAuthoritative())
			{
				go->setGcwImperialScorePercentileFromOtherGalaxy(msg.getValue().first, msg.getValue().second.first, msg.getValue().second.second);
			}
		}
	}
	else if (message.isType("LLTStatReq"))
	{
		ri = static_cast<GameNetworkMessage const &>(message).getByteStream().begin();
		...
		{
			lastLoginTimeStatistics[(60 * 60 * 24 * 1)]    = std::make_pair(std::string("totalCharacters.loginWithinPast_Day001"), 0);
			lastLoginTimeStatistics[(60 * 60 * 24 * 2)]    = std::make_pair(std::string("totalCharacters.loginWithinPast_Day002"), 0);
			lastLoginTimeStatistics[(60 * 60 * 24 * 3)]    = std::make_pair(std::string("totalCharacters.loginWithinPast_Day003"), 0);
			lastLoginTimeStatistics[(60 * 60 * 24 * 4)]    = std::make_pair(std::string("totalCharacters.loginWithinPast_Day004"), 0);
			lastLoginTimeStatistics[(60 * 60 * 24 * 5)]    = std::make_pair(std::string("totalCharacters.loginWithinPast_Day005"), 0);
			lastLoginTimeStatistics[(60 * 60 * 24 * 6)]    = std::make_pair(std::string("totalCharacters.loginWithinPast_Day006"), 0);
			lastLoginTimeStatistics[(60 * 60 * 24 * 7)]    = std::make_pair(std::string("totalCharacters.loginWithinPast_Day007"), 0);
			lastLoginTimeStatistics[(60 * 60 * 24 * 14)]   = std::make_pair(std::string("totalCharacters.loginWithinPast_Day014"), 0);
			lastLoginTimeStatistics[(60 * 60 * 24 * 21)]   = std::make_pair(std::string("totalCharacters.loginWithinPast_Day021"), 0);
			lastLoginTimeStatistics[(60 * 60 * 24 * 30)]   = std::make_pair(std::string("totalCharacters.loginWithinPast_Day030"), 0);
			lastLoginTimeStatistics[(60 * 60 * 24 * 60)]   = std::make_pair(std::string("totalCharacters.loginWithinPast_Day060"), 0);
			lastLoginTimeStatistics[(60 * 60 * 24 * 90)]   = std::make_pair(std::string("totalCharacters.loginWithinPast_Day090"), 0);
			lastLoginTimeStatistics[(60 * 60 * 24 * 180)]  = std::make_pair(std::string("totalCharacters.loginWithinPast_Day180"), 0);
			lastLoginTimeStatistics[(60 * 60 * 24 * 270)]  = std::make_pair(std::string("totalCharacters.loginWithinPast_Day270"), 0);
			lastLoginTimeStatistics[(60 * 60 * 24 * 365)]  = std::make_pair(std::string("totalCharacters.loginWithinPast_Year1"), 0);
			lastLoginTimeStatistics[(60 * 60 * 24 * 730)]  = std::make_pair(std::string("totalCharacters.loginWithinPast_Year2"), 0);
			lastLoginTimeStatistics[(60 * 60 * 24 * 1095)] = std::make_pair(std::string("totalCharacters.loginWithinPast_Year3"), 0);
		...
		std::map<int, std::pair<std::string, int> >::iterator iter = lastLoginTimeStatistics.begin();
		std::map<int, std::pair<std::string, int> >::iterator previousIter = lastLoginTimeStatistics.begin();
		...
		{
			createTimeStatistics[(60 * 60 * 24 * 1)]    = std::make_pair(std::string("totalCharacters.createdWithinPast_Day001"), 0);
			createTimeStatistics[(60 * 60 * 24 * 2)]    = std::make_pair(std::string("totalCharacters.createdWithinPast_Day002"), 0);
			createTimeStatistics[(60 * 60 * 24 * 3)]    = std::make_pair(std::string("totalCharacters.createdWithinPast_Day003"), 0);
			createTimeStatistics[(60 * 60 * 24 * 4)]    = std::make_pair(std::string("totalCharacters.createdWithinPast_Day004"), 0);
			createTimeStatistics[(60 * 60 * 24 * 5)]    = std::make_pair(std::string("totalCharacters.createdWithinPast_Day005"), 0);
			createTimeStatistics[(60 * 60 * 24 * 6)]    = std::make_pair(std::string("totalCharacters.createdWithinPast_Day006"), 0);
			createTimeStatistics[(60 * 60 * 24 * 7)]    = std::make_pair(std::string("totalCharacters.createdWithinPast_Day007"), 0);
			createTimeStatistics[(60 * 60 * 24 * 14)]   = std::make_pair(std::string("totalCharacters.createdWithinPast_Day014"), 0);
			createTimeStatistics[(60 * 60 * 24 * 21)]   = std::make_pair(std::string("totalCharacters.createdWithinPast_Day021"), 0);
			createTimeStatistics[(60 * 60 * 24 * 30)]   = std::make_pair(std::string("totalCharacters.createdWithinPast_Day030"), 0);
			createTimeStatistics[(60 * 60 * 24 * 60)]   = std::make_pair(std::string("totalCharacters.createdWithinPast_Day060"), 0);
			createTimeStatistics[(60 * 60 * 24 * 90)]   = std::make_pair(std::string("totalCharacters.createdWithinPast_Day090"), 0);
			createTimeStatistics[(60 * 60 * 24 * 180)]  = std::make_pair(std::string("totalCharacters.createdWithinPast_Day180"), 0);
			createTimeStatistics[(60 * 60 * 24 * 270)]  = std::make_pair(std::string("totalCharacters.createdWithinPast_Day270"), 0);
			createTimeStatistics[(60 * 60 * 24 * 365)]  = std::make_pair(std::string("totalCharacters.createdWithinPast_Year1"), 0);
			createTimeStatistics[(60 * 60 * 24 * 730)]  = std::make_pair(std::string("totalCharacters.createdWithinPast_Year2"), 0);
			createTimeStatistics[(60 * 60 * 24 * 1095)] = std::make_pair(std::string("totalCharacters.createdWithinPast_Year3"), 0);
		...
		const GenericValueTypeMessage<std::pair<std::map<int, std::pair<std::string, int> >, std::map<int, std::pair<std::string, int> > > > lastLoginTimeStatisticsResponse("LLTStatRsp", std::make_pair(lastLoginTimeStatistics, createTimeStatistics));
		sendToCentralServer(lastLoginTimeStatisticsResponse);
	}
	else if (message.isType("LfgStatReq"))
	{
		ri = static_cast<GameNetworkMessage const &>(message).getByteStream().begin();
		...
		if (numberOfCharacterMatchRequests || numberOfCharacterMatchResults || timeSpentOnCharacterMatchRequestsMs)
		{
	...
	else if (message.isType("TransferReplyMoveValidation"))
	{
		ri = static_cast<GameNetworkMessage const &>(message).getByteStream().begin();
	...
	else if (message.isType("CreateDynamicSpawnRegionCircleMessage"))
	{
		ri = static_cast<GameNetworkMessage const &>(message).getByteStream().begin();
		CreateDynamicSpawnRegionCircleMessage const msg(ri);

		RegionMaster::createNewDynamicRegionWithSpawn(msg.getCenterX(), msg.getCenterY(), msg.getCenterZ(), msg.getRadius(), msg.getName(), msg.getPlanet(),
													  msg.getPvp(), msg.getBuildable(), msg.getMunicipal(), msg.getGeography(), msg.getMinDifficulty(), msg.getMaxDifficulty(),
													  msg.getSpawnable(), msg.getMission(), msg.getVisible(), msg.getNotify(), msg.getSpawntable(), msg.getDuration());
	}
}
